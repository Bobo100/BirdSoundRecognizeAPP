...
public class AudioFeatureExtraction {
    ...
    private int n_mfcc = 256;
    private double sampleRate = 44100.0;
    private int length = 1;
    private double fMax = 15000.0;
    private double fMin = 500.0;
    private int n_fft = 1024;
    private int hop_length = 172;
...
//        this.fMax = this.sampleRate / 2.0;
    ...
    private double[][] dctMfcc(float[] y) {
        double[][] melS = melSpectrogram(y);
        final double[][] specTroGram = powerToDb(melS);
    ...
    public float[][] melSpectrogramWithComplexValueProcessing(float[] y) {
        ...
        for (int i = 0; i < spectro.length; i++) {
            for (int j = 0; j < spectro[0].length; j++) {
                ...
                spectroAbsVal[i][j] = Math.pow(spectroDblVal, 1);
            }
        }
...
//        final int n_frames = (ypad.length - n_fft) / hop_length;
...
//        }
        ...
        for (int i = 0; i < melS.length; i++) {
            for (int j = 0; j < melS[0].length; j++) {
                double magnitude = Math.abs(melS[i][j]);
                if (magnitude > 1e-10) {
//                    log_spec[i][j] = 10.0 * log10_2(magnitude, max);
...
//        }
    ...
    public double[][] amplitudeToDb(double[][] melS) {
        // Convert a power spectrogram (amplitude squared) to decibel (dB) units
        // This computes the scaling ``20 * log10(S / ref)`` in a numerically
        // stable way.
        double[][] log_spec = new double[melS.length][melS[0].length];
        double maxValue = -100;
...
//            for (int j = 0; j < melS[0].length; j++) {
//                double magnitude = Math.abs(melS[i][j]);
//                if (magnitude > 1e-10 && max > 1e-10) {
//                    log_spec[i][j] = 20.0 * log10_2(magnitude, max);
////                    log_spec[i][j] = 20.0 * log10(magnitude);
//                } else {
//                    log_spec[i][j] = 10.0 * (-10);
////                    log_spec[i][j] = 0;
//                }
...
//                    log_spec[i][j] = maxValue - 80.0;
//                }
////                if (log_spec[i][j] < max - 80.0) {
////                    log_spec[i][j] = max - 80.0;
////                }
//            }
//        }
//
...
////                double magnitude = Math.abs(melS[i][j]);
////                if (max == 0) {
////                    log_spec[i][j] = 20.0 * log10_2(magnitude, magnitude);
////                }
////            }
////        }
        ...
        for (int i = 0; i < melS.length; i++) {
            for (int j = 0; j < melS[0].length; j++) {
                ...
                if (magnitude > 1e-10) {
                    ...
                    log_spec[i][j] = 20.0 * log10(1e-10);
                }
                if (max > 1e-10) {
                    log_spec[i][j] -= 20.0 * log10(max);
                }else {
                    log_spec[i][j] -= 20.0 * log10(1e-10);
                }
            }
...
//        log_spec -= 20.0 * np.log10(np.maximum(amin, ref_value))
//
...
//        log_spec = np.maximum(log_spec, log_spec.max() - top_db)
    ...
    private double log10_2(double value, double ref) {
        return Math.log10(value / ref);

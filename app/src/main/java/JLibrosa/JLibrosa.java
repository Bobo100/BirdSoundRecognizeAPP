...
import JLibrosa.exception.FileFormatNotSupportedException;
import JLibrosa.process.AudioFeatureExtraction;
import JLibrosa.wavFile.WavFile;
import JLibrosa.wavFile.WavFileException;
...
//	private int sampleRate = -1;
	...
	private final int n_fft = 1024;
	private final int hop_length = 172;
	private final int n_mels = 256;
	...
	public float [] generateMeanMFCCFeatures(float[][] mfccValues, int nMFCC, int nFFT) {
		...
		for (int i=0; i<mfccValues.length; i++) {
			...
			if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
				ds = IntStream.range(0, floatArrValues.length)
